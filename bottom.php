<style type="text/css">
	/* FOOTER */

	footer {
		background: url("/jacobmor/static/img/new-york-diamond-district.jpg");
		background-size: cover;
		background-position: top;
	}

	.d-hero {
		color: white;
		text-align: center;
	 	display: flex; 
	    justify-content: center; 
	    flex-direction: column; 
	    padding-top: 128px !important;
	    padding-bottom: 128px !important;
	    line-height: 30px;
	    font-size: 18px;
	}

	@media (max-width: 767px) {
		.d-hero {
		    padding-top: 80px !important;
		    padding-bottom: 80px !important;
		}
	}

	.footer {
		background: none;
		background-color: rgba(0,0,0,0.4); 
		color: white;
		text-align: center;
	 	display: flex; 
	    justify-content: center; 
	    flex-direction: column; 
	    padding-top: 64px !important;
	    padding-bottom: 64px !important;
	    line-height: 30px;
	    font-size: 18px;
	}

	@media (max-width: 767px) {
		.footer {
		    padding-top: 49px !important;
		    padding-bottom: 49px !important;
		}
	}


	.footer-text {
		color: #7B8080;
	}

	.copyright {
		margin-top: 28px; 
	}

</style>

<!-- FOOTER -->
<footer>
	<div class="container-fluid d-hero" >
		<div class="row">
			<div class="col-lg-3 col-md-2">
			</div>
			<div class="col-lg-6 col-md-8">
				<h2>Diamonds You Can Trust</h2>
				<img src="/jacobmor/static/img/hr.svg">
				<p>JacobMor.com is proudly offered by Abe Mor. From high above Manhattan’s bustling Diamond District, we’ve served the jewelry market with trust, fairness and quality for more than 50 years. Now, we’re delighted to guide you through purchases you’ll feel great about while building a lasting relationship. Please contact us by phone at (212) 921-4220 or email at hello@abemor.com for any reason, any time.</p>
			</div>
		</div>
	</div>

	<div class="container-fluid footer">
		<div class="row">
			<div class="col-lg-3 col-md-2">
			</div>
			<div class="col-lg-6 col-md-8">
				<h2>Abe Mor Diamonds</h2>
				<p class="footer-text">62 West 47th Street, #401, New York, NY 10036<br>(212) 921-4220  hello@jacobmor.com</p>
				<p> HOME&nbsp;&nbsp;&nbsp;<span class="footer-text">/</span>&nbsp;&nbsp;&nbsp;TERMS & CONDITIONS&nbsp;&nbsp;&nbsp;<span class="footer-text">/</span>&nbsp;&nbsp;&nbsp;CONTACT US</p>
				<p class="footer-text copyright">Copyright ©2016 Abe Mor Diamond Cutters & Co.</p>
			</div>
		</div>
	</div>
</footer>


</html>